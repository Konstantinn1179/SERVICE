...
const BookingForm: React.FC<Props> = ({ onSubmit, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-gray-900 w-full max-w-sm rounded-2xl flex flex-col shadow-2xl border border-gray-700 relative overflow-hidden">
        ...
        <div className="p-6">
          <div className="flex justify-between items-start mb-6">
            <div>
               ...
               <p className="text-xs text-gray-500 mt-1">Оставьте контакт для связи с мастером</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs text-gray-400 mb-1 ml-1">Как к вам обращаться?</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="Иван Иванов"
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-gray-600 transition-all"
              />
            </div>

            <div>
              <label className="block text-xs text-gray-400 mb-1 ml-1">Номер телефона</label>
              <input
                type="tel"
                value={phone}
                onChange={handlePhoneChange}
                placeholder="+7 (999) 000-00-00"
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-gray-600 transition-all font-mono"
              />
            </div>
